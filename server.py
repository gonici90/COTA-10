...
def home_extended():
    ...
    js = r'''async function syncData(){let o=out;o.innerHTML='<p>Actualizez baza automată...</p>';try{let r=await fetch('/api/data/sync?days=7',{method:'POST'}),x=await r.json();if(!r.ok)throw Error(x.detail||'Eroare');o.innerHTML='<div class="card"><div class="prob">Date actualizate</div><b>'+val(x.matches,0)+' meciuri</b> · '+val(x.finished,0)+' terminate<br><small>'+val(x.api_days_fetched,0)+' zile luate din API · '+val(x.cached_days,0)+' din cache</small></div>'}catch(e){o.innerHTML='<div class="card warn">Eroare actualizare: '+e.message+'</div>'}}
