...
Community Access: fetch fixtures first, then real Bet365 markets per fixture.
"""
import math
...
def _ah_prob(g,line,home_side=True):
    ...
    for h in range(11):
        for a in range(11):
            x=((h-a) if home_side else (a-h))+line
            p+=(g[h][a] if x>0 else .5*g[h][a] if abs(x)<1e-9 else 0)
    return p
...
def _team_rates(team_id,games):
    ...
    for i,m in enumerate(games):
        teams=m.get('teams') or {}; home=(teams.get('home') or {}).get('id')==team_id; goals=m.get('goals') or {}; corners=m.get('corners') or {}; cards=m.get('cards') or {}
        hg,ag=_num(goals.get('home')),_num(goals.get('away')); hc,ac=_num(corners.get('home')),_num(corners.get('away')); hca,aca=cards.get('home') or {},cards.get('away') or {}
        if hg is None or ag is None:continue
        hy=(_num(hca.get('yellow')) or 0)+2*(_num(hca.get('red')) or 0); ay=(_num(aca.get('yellow')) or 0)+2*(_num(aca.get('red')) or 0); w=.90**i; ws+=w
...
def analyze_fixture(f):
    teams=f.get('teams') or {}; h=teams.get('home') or {}; a=teams.get('away') or {}; fid=f.get('id'); ts=f.get('kickoff_ts') or int(datetime.now(timezone.utc).timestamp())
    hr=_team_rates(h.get('id'),_recent(h.get('id'),ts)); ar=_team_rates(a.get('id'),_recent(a.get('id'),ts)); lh=max(.15,(hr['gf']+ar['ga'])/2*1.06); la=max(.15,(ar['gf']+hr['ga'])/2*.96); hp,dp,ap,g=_score_probs(lh,la); odds=_odds_payload(fid); picks=[]
    ...
    m=_stage(odds.get('goal_line') or odds.get('goals'))
    if m:
        line=_num(m.get('line'))
        if line is not None:_add(picks,f'Over {line:g}',_total_prob(lh+la,line,True),m.get('over'),'Bet365 Goal Line');_add(picks,f'Under {line:g}',_total_prob(lh+la,line,False),m.get('under'),'Bet365 Goal Line')
    corner_lam=max(2,(hr['cf']+hr['ca']+ar['cf']+ar['ca'])/2); m=_stage(odds.get('corner_line') or odds.get('corners'))
    if m:
        line=_num(m.get('line'))
        if line is not None:_add(picks,f'Corners Over {line:g}',_total_prob(corner_lam,line,True),m.get('over'),'Bet365 Corners');_add(picks,f'Corners Under {line:g}',_total_prob(corner_lam,line,False),m.get('under'),'Bet365 Corners')
    card_lam=max(.8,(hr['cards_for']+hr['cards_against']+ar['cards_for']+ar['cards_against'])/2); m=_stage(odds.get('card_line') or odds.get('cards'))
    if m:
        line=_num(m.get('line'))
        if line is not None:_add(picks,f'Cards Over {line:g}',_total_prob(card_lam,line,True),m.get('over'),'Bet365 Cards');_add(picks,f'Cards Under {line:g}',_total_prob(card_lam,line,False),m.get('under'),'Bet365 Cards')
...
def analyze_day(day,target=10,limit=12):
    ...
    return {'date':day,'provider':'5DollarFootballAPI + Bet365','api_fixtures':len(fs),'eligible':len(fs),'analyzed':len(rows),'analysis_errors':errors[:10],'ranking':rows,'suggested_combo':build_combo(rows,float(target))}