...
app=FastAPI(title='COTA 10 Football Analyzer',version='5.1')
...
@app.get('/health')
def health():return {'status':'ok','provider':'footballdata.io','api_key_configured':bool(API_KEY),'version':'5.1'}
@app.get('/api/analyze')
...
async def backtest(days:int=Query(7,ge=1,le=30),per_day:int=Query(5,ge=1,le=10)):
 ...
 for off in range(days,0,-1):
  ...
  try:fs=extract_matches(await api_get(f'/matches/date/{d}',{'limit':100}))
  except Exception as e:errs.append({'date':d,'error':str(e)[:160]});continue
  for f in [x for x in fs if None not in score_values(x)][:per_day]:
   try:
    a=await analyze_fixture(f,d,False);h,aw=score_values(f);pk=a['best_market'];p=pk['probability']
    if p<60 or a['confidence']=='scăzută':continue
    w=market_won(pk['market'],h,aw);bk='90+' if p>=90 else '80-89' if p>=80 else '70-79' if p>=70 else '60-69';buckets[bk]['n']+=1;buckets[bk]['wins']+=int(w);s=stats.setdefault(pk['market'],{'n':0,'wins':0,'prob_sum':0.});s['n']+=1;s['wins']+=int(w);s['prob_sum']+=p;picks.append({'date':d,'match':a['home']+' - '+a['away'],'market':pk['market'],'probability':p,'result':f'{h}-{aw}','won':w})
   except Exception as e:
    if '429' in str(e):errs.append({'date':d,'error':'API rate limit atins; backtest parțial.'});break
 for s in stats.values():s['hit_rate']=round(100*s['wins']/s['n'],1);s['avg_predicted']=round(s['prob_sum']/s['n'],1);s['calibration_gap']=round(s['avg_predicted']-s['hit_rate'],1);del s['prob_sum']
