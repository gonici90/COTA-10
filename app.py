...
app = FastAPI(title="COTA 10 Football Analyzer", version="4.0")
...
async def analyze_fixture(f,day):
    ...
    for name,p in market_probabilities(hl,al).items():
        ...
        p=max(.05,min(.95,p)); fair=1/p; book=bookmaker.get(name); ev=((p*book)-1)*100 if book else None
        markets.append({"market":name,"probability":round(p*100,1),"fair_odds":round(fair,2),"bookmaker_odds":round(book,2) if book else None,"ev":round(ev,1) if ev is not None else None,"value":bool(ev is not None and ev>=MIN_VALUE_EV)})
    markets.sort(key=lambda x:(x["value"],x["ev"] if x["ev"] is not None else -999,x["probability"]),reverse=True)
    best_value=next((x for x in markets if x["value"]),None); best_market=best_value or max(markets,key=lambda x:x["probability"]); league=f.get("league") or {}
...
def build_target_combo(matches,target=10.0):
    ...
    for m in matches:
        ...
        choices=[x for x in m["markets"] if x["value"] and x["probability"]>=MIN_COMBO_PROBABILITY and x["bookmaker_odds"]]
        if choices:
            best=max(choices,key=lambda x:(x["ev"],x["probability"])); candidates.append({**m,"selection":best["market"],"probability":best["probability"],"odds":best["bookmaker_odds"],"ev":best["ev"]})
    ...
    for size in range(2,min(8,len(candidates))+1):
        for combo in combinations(candidates,size):
            ...
            avg=sum(x["probability"] for x in combo)/size; avg_ev=sum(x["ev"] for x in combo)/size; distance=abs(math.log(max(product,.01)/target)); score=distance-avg/800-avg_ev/500
            if best is None or score<best[0]:best=(score,product,combo)
    if not best:return None
    return {"combined_bookmaker_odds":round(best[1],2),"matches":list(best[2]),"minimum_probability":MIN_COMBO_PROBABILITY,"minimum_ev":MIN_VALUE_EV}

@app.get("/health")
def health():return {"status":"ok","provider":"footballdata.io","api_key_configured":bool(API_KEY),"version":"4.0"}
...
async def analyze(day:str=Query(default_factory=lambda:date.today().isoformat()),limit:int=8,target:float=10.0):
    ...
    return {"date":day,"market":"Multi-market + EV","api_fixtures":len(fixtures),"eligible":len(upcoming),"analyzed":len(results),"analysis_errors":errors[:10],"target":target,"ranking":results,"suggested_combo":build_target_combo(results,target),"combo_min_probability":MIN_COMBO_PROBABILITY,"min_value_ev":MIN_VALUE_EV}

@app.get("/",response_class=HTMLResponse)
def home():
 return '''<!doctype html><html lang="ro"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>COTA 10</title><style>body{font-family:system-ui;background:#0b0f14;color:#fff;margin:0;padding:24px}main{max-width:900px;margin:auto}.card{background:#151b23;border:1px solid #29313d;border-radius:16px;padding:18px;margin:14px 0}button{background:#35c46a;border:0;border-radius:10px;padding:13px 18px;font-weight:800}input{padding:12px;border-radius:9px;border:1px solid #394453;background:#0b0f14;color:white}small{color:#9ba8b7}.prob{font-size:24px;font-weight:800}.warn{color:#ffcc66}.good{color:#55df8b}.tag{display:inline-block;padding:4px 8px;border-radius:8px;background:#263241;margin:4px}.value{background:#174a32;color:#76f0aa}</style></head><body><main><h1>COTA 10</h1><p>Analizor multi-piață + cote bookmaker + EV. Estimările nu garantează rezultate.</p><div class="card"><input id="d" type="date"><input id="t" type="number" value="10" step="0.1"><button onclick="go()">Analizează</button></div><div id="out"></div></main><script>document.getElementById('d').value=new Date().toISOString().slice(0,10);async function go(){let o=document.getElementById('out');o.innerHTML='<p>Analizez probabilități, cote și value...</p>';try{let r=await fetch('/api/analyze?day='+d.value+'&target='+t.value);let x=await r.json();if(!r.ok)throw new Error(x.detail||'Eroare');let h='<div class="card"><b>'+x.analyzed+' meciuri analizate</b><br><small>API: '+x.api_fixtures+' · eligibile: '+x.eligible+' · prag value EV +'+x.min_value_ev+'%</small></div>';if(x.suggested_combo){h+='<h2>Bilet VALUE ~ cota '+t.value+'</h2><div class="card"><b>Cotă bookmaker combinată: '+x.suggested_combo.combined_bookmaker_odds+'</b><br>'+x.suggested_combo.matches.map(m=>'<b>'+m.home+' – '+m.away+'</b>: '+m.selection+' @ '+m.odds+' · '+m.probability+'% · <span class="good">EV +'+m.ev+'%</span>').join('<br>')+'</div>'}else h+='<div class="card warn"><b>Nu există suficiente selecții +EV pentru un bilet solid la ținta aleasă.</b></div>';h+='<h2>Analiza multi-piață + VALUE</h2>';for(let m of x.ranking){let b=m.best_market;h+='<div class="card"><small>'+m.country+' · '+m.league+'</small><h3>'+m.home+' – '+m.away+'</h3><div class="prob">'+(m.best_value?'VALUE: ':'Recomandare: ')+b.market+' · '+b.probability+'%</div><small>xG '+m.home_xg+' - '+m.away_xg+' · istoric '+m.home_last+'/'+m.away_last+' · încredere '+m.confidence+'</small><br>'+(b.bookmaker_odds?'<b>Cotă bookmaker '+b.bookmaker_odds+' · fair '+b.fair_odds+' · <span class="'+(b.ev>=3?'good':'')+'">EV '+(b.ev>=0?'+':'')+b.ev+'%</span></b>':'<span class="warn">Cote bookmaker indisponibile</span>')+'<br>'+m.markets.slice(0,10).map(z=>'<span class="tag '+(z.value?'value':'')+'">'+z.market+' '+z.probability+'%'+(z.bookmaker_odds?' @'+z.bookmaker_odds+' EV '+(z.ev>=0?'+':'')+z.ev+'%':'')+'</span>').join('')+'</div>'}o.innerHTML=h}catch(e){o.innerHTML='<div class="card warn"><b>Eroare:</b> '+e.message+'</div>'}}</script></body></html>'''