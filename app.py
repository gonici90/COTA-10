...
app = FastAPI(title="COTA 10 Football Analyzer", version="1.0")
...
async def api_get(path: str, params: dict):
    ...
    if r.status_code != 200:
        raise HTTPException(502, f"API-Football returned HTTP {r.status_code}")
    data = r.json()
    if data.get("errors"):
...
async def recent_team_stats(team_id: int, last: int = 8):
    games = await api_get("/fixtures", {"team": team_id, "last": last})
...
async def analyze_fixture(f):
    home = f["teams"]["home"]
    away = f["teams"]["away"]
    hs = await recent_team_stats(home["id"])
    aws = await recent_team_stats(away["id"])
    ...
    probability = 0.70 * poisson + 0.30 * form_rate
    ...
    return {
        ...
        "over25_probability": round(probability * 100, 1),
        "fair_odds": round(fair_odds, 2),
        "home_last": hs["played"],
        "away_last": aws["played"],
    }
