...
app = FastAPI(title="COTA 10 Football Analyzer", version="2.2")
...
async def recent_team_stats(team_id: int, before_day: str, venue: str, last: int = 10):
    ...
    def calc(sample):
        ...
        gf = ga = ov = btts = 0.0; weight_sum = 0.0
        for i, g in enumerate(sample):
            ...
            ov += int(h + a >= 3) * weight; btts += int(h > 0 and a > 0) * weight; weight_sum += weight
...
async def analyze_fixture(f, day: str):
    ...
    home_lambda=max(.15,((hs["gf"]+aws["ga"])/2)*1.07); away_lambda=max(.15,((aws["gf"]+hs["ga"])/2)*.96)
    expected=max(.5,min(5.5,home_lambda+away_lambda)); poisson=poisson_over_25(expected)
    form_over=(hs["over25_rate"]+aws["over25_rate"])/2; btts=(hs["btts_rate"]+aws["btts_rate"])/2
    probability=.72*poisson+.20*form_over+.08*btts; sample=min(hs["played"],aws["played"])
    if sample<5: probability=.75*probability+.25*.50
    probability=max(.08,min(.90,probability)); league=f.get("league") or {}
...
def build_target_combo(matches,target=10.0):
    candidates=[x for x in sorted(matches,key=lambda x:x["over25_probability"],reverse=True) if x["confidence"]!="scăzută" and x["over25_probability"]>=MIN_COMBO_PROBABILITY][:16]
    best=None
    for size in range(2,min(8,len(candidates))+1):
        for combo in combinations(candidates,size):
            product=math.prod(x["fair_odds"] for x in combo)
            # Do not force a weak/undersized ticket just to imitate the requested target.
            if product < target * .75: continue
            distance=abs(math.log(max(product,.01)/target)); avg_prob=sum(x["over25_probability"] for x in combo)/size
            score=distance-avg_prob/1000
...
@app.get("/health")
def health(): return {"status":"ok","provider":"footballdata.io","api_key_configured":bool(API_KEY),"version":"2.2"}
