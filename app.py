...
app = FastAPI(title="COTA 10 Football Analyzer", version="4.1")
...
async def analyze_fixture(f,day):
    ...
    markets.sort(key=lambda x:(x["value"],x["ev"] if x["ev"] is not None else -999,x["probability"]),reverse=True);best_value=next((x for x in markets if x["value"]),None);best_market=best_value or max(markets,key=lambda x:x["probability"]);league=f.get("league") or {}
...
def build_target_combo(matches,target=10.0):
    ...
    for m in matches:
        ...
        if not choices:choices=[x for x in m["markets"] if x["probability"]>=MIN_COMBO_PROBABILITY and x["fair_odds"]>=1.18]
        if choices:
            best=max(choices,key=lambda x:((x["ev"] or -999) if x["value"] else x["probability"]));candidates.append({"home":m["home"],"away":m["away"],"selection":best["market"],"probability":best["probability"],"odds":best["bookmaker_odds"],"fair_odds":best["fair_odds"],"ev":best["ev"],"value":best["value"]})
    candidates=sorted(candidates,key=lambda x:(x["value"],x["ev"] or -999,x["probability"]),reverse=True)[:16];best=None
    for size in range(2,min(8,len(candidates))+1):
        for combo in combinations(candidates,size):
            product=math.prod((x["odds"] or x["fair_odds"]) for x in combo)
    ...
    return {"combined_odds":round(best[1],2),"matches":list(best[2]),"uses_bookmaker_odds":all(x["odds"] for x in best[2])}
@app.get("/health")
def health():return {"status":"ok","provider":"footballdata.io","api_key_configured":bool(API_KEY),"version":"4.1"}
@app.get("/api/analyze")
