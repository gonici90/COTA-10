"""COTA 10 v20.3: sportsbook browser + deep multimarket on-demand analysis."""
...
def install(app):
 ...
 def match_analysis(fixture_id:str,day:str):
  ...
  fid=found.get('id') or found.get('fixture_id');odds={};source='bulk'
  inline=found.get('odds') or found.get('markets')
  if inline:odds=market_engine._normalize_odds({'odds':inline})
  # IMPORTANT: bulk often contains only 1X2. For an explicitly selected match we
  # always request the full Bet365 odds payload, then merge all returned markets.
  if fid:
   try:
    deep=market_engine._normalize_odds(fd._get(f'/fixtures/{fid}/odds',{'bookmakers':'bet365'}))
    if isinstance(deep,dict) and deep:
     merged=dict(odds);merged.update(deep);odds=merged;source='fixture-detail-full'
   except Exception:pass
  enriched=dict(found);enriched.pop('_pro_bulk_odds',None)
