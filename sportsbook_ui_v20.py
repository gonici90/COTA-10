"""COTA 10 v20 sportsbook UX: browse first, analyze one match on demand."""
...
def install(app):
    ...
    def fixtures(day:str, days:int=Query(1,ge=1,le=7)):
        ...
        for i in range(days):
            ...
            for f in market_engine._day_fixtures(d):
                if not isinstance(f,dict): continue
                fid=f.get('id') or f.get('fixture_id'); h,a=market_engine._teams(f)
                key=fid or (h.get('name'),a.get('name'),market_engine._kickoff_ts(f))
                if key in seen: continue
                seen.add(key)
                league=f.get('league') or {}; country=str(league.get('country') or f.get('country') or 'Altele'); comp=str(league.get('name') or f.get('league_name') or 'Competiție')
                groups[(country,comp)].append({'id':fid,'home':h.get('name','?'),'away':a.get('name','?'),'kickoff':market_engine._kickoff_ts(f),'date':d})
        return {'day':day,'count':sum(map(len,groups.values())),'competitions':[{'country':k[0],'competition':k[1],'matches':v} for k,v in sorted(groups.items())]}

    @app.get('/api/match-analysis')
    def match_analysis(fixture_id:str, day:str):
        found=None
        for f in market_engine._day_fixtures(day):
            if str(f.get('id') or f.get('fixture_id'))==str(fixture_id): found=f; break
        if found is None: raise HTTPException(404,'Meciul nu a fost găsit')
        # One match only: live analysis can afford the independent model/history path.
        try:r=market_engine.analyze_fixture(found)
        except Exception as e: raise HTTPException(502,'Analiza meciului a eșuat: '+str(e)[:160])
        if not r.get('best_market'): raise HTTPException(422,'Nu există suficiente date/cote pentru acest meci')
        return r
...
async function analyze(btn,id,d){let box=btn.parentElement.querySelector('div[id^=r]');btn.disabled=true;btn.textContent='ANALIZEZ…';box.className='result';box.innerHTML='Analizez doar acest meci…';try{let r=await fetch('/api/match-analysis?fixture_id='+encodeURIComponent(id)+'&day='+d);let j=await r.json();if(!r.ok)throw Error(j.detail||'Eroare');let b=j.best_market||{};box.innerHTML='<div class="best">'+esc(b.market)+' @'+esc(b.bookmaker_odds)+' • '+esc(b.ticket_probability||b.probability)+'%</div><div class="time">xG '+esc(j.xg_home)+' – '+esc(j.xg_away)+'</div><div class="markets">'+(j.markets||[]).slice(0,12).map(x=>'<div class="market"><span>'+esc(x.market)+'</span><span>@'+esc(x.bookmaker_odds)+' • '+esc(x.ticket_probability||x.probability)+'%</span></div>').join('')+'</div>'}catch(e){box.innerHTML='<span class="err">'+esc(e.message)+'</span>'}finally{btn.disabled=false;btn.textContent='REANALIZEAZĂ'}}load();
</script></body></html>'''
